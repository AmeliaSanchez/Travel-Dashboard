...
function createTripCard(trip) {
  ...
  return `<details class="trip-card">
    <summary class="trip-summary">
      <span class="destination"><span class="flag" aria-hidden="true">${escapeHtml(flagFor(trip))}</span><span><span class="destination-name">${escapeHtml(trip.destino)}</span><span class="destination-country">${escapeHtml(trip.pais)}</span></span></span>
...
async function init() {
  try {
    ...
    try { state.data = JSON.parse(localStorage.getItem(STORAGE_KEY)) || sourceData; }
